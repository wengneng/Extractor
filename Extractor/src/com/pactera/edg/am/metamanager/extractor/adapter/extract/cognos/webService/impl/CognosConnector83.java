 ...
 * 
 */
public class CognosConnector83 {
	private Log log = LogFactory.getLog(CognosConnector83.class);
	// 创建连接Cognos8服务的对象.
	 ...
	 */
	public ContentManagerService_Port connectByWebService() {
		// BI Bus
		...
		try {
			...
			cmService = cmServiceLocator.getcontentManagerService(serverURL);
				new ContentManagerServiceStub(serverURL,
					cmServiceLocator);
	 ...
	 */
	public void  logOff(){
		if(getCmService()!=null){
			try {
				getCmService().logoff();
				log.info("注销cognos服务器.");
			} catch (RemoteException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public void setCmService(ContentManagerService_Port cmService) {
