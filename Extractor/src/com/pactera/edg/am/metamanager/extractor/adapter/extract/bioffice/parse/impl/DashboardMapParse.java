 ...
 * 
 */
public class DashboardMapParse extends AbstractParse {

	private static Log log = LogFactory.getLog(DashboardMapParse.class);
	...
	public BofBO parse(BofBO parent, ICatalogElement element) {
		log.debug("enter DashboardMapParse parse");
		Date date1=new Date();
		...
		try {
			...
			for (int i = 0; i < fields.size(); i++) {
				...
				ICatalogElement fieldElement = catalogService.getCatalogElementById(field.getId());
				if (fieldElement != null) {
					// 报表字段的id采用：父结点id+">"+报表字段id+">DashboardMapField>"+序号（从1开始计算）
					BofBO fieldBofBO = new BofBO(bofBO, bofBO.getId() + ">" + fieldElement.getId() + ">DashboardMapField>"
							+ (i + 1));
