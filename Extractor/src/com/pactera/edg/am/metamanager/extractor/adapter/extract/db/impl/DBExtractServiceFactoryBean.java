 ...
 */
public class DBExtractServiceFactoryBean implements FactoryBean, BeanClassLoaderAware, InitializingBean {
	 ...
	 */
	private Map<String, IDBExtractService> dbExtractors = new HashMap<String, IDBExtractService>();

	public Map<String, IDBExtractService> getDbExtractors() {
		return dbExtractors;
	}

	public void setDbExtractors(Map<String, IDBExtractService> dbExtractors) {
	...
	public Object getObject() throws Exception {
		String name = getDatabaseProductName();
		if (name != null && dbExtractors.containsKey(name)) {// 名称全匹配的情况
			return dbExtractors.get(name);
		}
		for (Iterator<String> it = dbExtractors.keySet().iterator(); it.hasNext();) {
			String key = it.next();
	...
	public Class<?> getObjectType() {
		return IDBExtractService.class;
	}
